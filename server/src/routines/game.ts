...
function getAvailableEnergy(game: GameModel) {
	...
	if (activeRow) {
		...
		for (let i = 0; i < activeRow.itemCards.length; i++) {
			...
			availableEnergy.push(...card.getEnergy(game, pos))
		}
...
function getAvailableActions(game: GameModel, availableEnergy: Array<EnergyT>): TurnActions {
	...
	if (activeRow !== null) {
		...
		if (activeRow !== null && turnState.turnNumber > 1) {
			...
			if (row.hermitCard && row.hermitCard.hasAttacks()) {
				if (hasEnoughEnergy(availableEnergy, row.hermitCard.props.primary.cost)) {
					actions.push('PRIMARY_ATTACK')
				}
				if (hasEnoughEnergy(availableEnergy, row.hermitCard.props.secondary.cost)) {
	...
	if (activeRow !== null || turnState.turnNumber <= 2) {
		...
		const desiredActions = currentPlayer.hand.reduce(
			(reducer: TurnActions, card: Card): TurnActions => {
				const pickableSlots = game.filterSlots(card.attachCondition)

				if (pickableSlots.length === 0) return reducer

				if (card.props.type === 'hermit' && !reducer.includes('PLAY_HERMIT_CARD')) {
					return [...reducer, 'PLAY_HERMIT_CARD']
				}
				if (card.props.type === 'effect' && !reducer.includes('PLAY_EFFECT_CARD')) {
					return [...reducer, 'PLAY_EFFECT_CARD']
				}
				if (card.props.type === 'item' && !reducer.includes('PLAY_ITEM_CARD')) {
					return [...reducer, 'PLAY_ITEM_CARD']
				}
				if (card.props.type === 'single_use' && !reducer.includes('PLAY_SINGLE_USE_CARD')) {
	...
	for (let playerState of playerStates) {
		...
		for (let rowIndex in playerRows) {
			...
			if (row.hermitCard && row.health <= 0) {
				const cardType = row.hermitCard.props.type
